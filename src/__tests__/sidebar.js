...
import SidebarDrawerDefault from '../components/Sidebar/SideDrawer/SideDrawer';
...
let wrapper; let wrapperInstance; let store; let initialState;

const { sidebarProps, userProfileMockData } = fileMock;
...
describe('Expect to mock user profile info', () => {
  ...
  it('Should test spinner loading status', () => {
    ...
    expect(store.getActions()).toEqual([userProfileMockData.spinnerStatusAction(false)]);
  });
  ...
  it('Should user profile fetching info fail', async () => {
    ...
    await store.dispatch(getProfile())
      .catch(async (error) => {
        const calledActions = store.getActions();
        expect(calledActions).toEqual([{ type: 'SPINNER_STATUS', spinner: false }]);
      });
  });
});
...
describe('Expect profile action reducer to render', () => {
  it('Should spinner actions tested in reducer', () => {
    const spinnerSiwtchCase = ProfileReducers({}, userProfileMockData.spinnerStatusAction('spinner loading'));
    expect(spinnerSiwtchCase).toEqual({ spinner: 'spinner loading' });
  });

  it('Should successfully fetch reducer work', () => {
    const expectedData = userProfileMockData.successResponse.profileInformation.data;
    const fetchSuccesfulyReducer = ProfileReducers({}, userProfileMockData.successfulyProfileReducer(expectedData));
  ...
  it('Should fail to fetch user profile reduce', () => {
    ...
    const fetchSuccesfulyReducer = ProfileReducers({}, userProfileMockData.failedProfileReducer(expectedData));
...
describe('Expect TopRightSide to render', () => {
  ...
  it('Expect TopRightSide to have props', () => {
    ...
    expect(wrapper5.instance().props.state).toMatchObject({ viewProfile: { profile: 'data' }, profileError: 'error' });
  });
...
describe('Expect to cover common map state to props ', () => {
  let wrapper6; let store2; let wrapper7; let wrapper8;
  ...
  beforeEach(() => {
    ...
    wrapper8 = shallow(<SidebarDrawerDefault store={store2} {...customProps}/>);
  });
  ...
  it('Should disaptch user profile data', () => {
    const dispatchProfileAction = store2.dispatch({ type: 'FETCH_PROFILE_SUCCESS', payload: 'new user info' });
  ...
  it('Should Sidebar map state to props covered', () => {
    expect(wrapper8.props().children.props.profile).toBe('data');
    expect(wrapper8.props().children.props.profileError).toBe('error');
  });
});
